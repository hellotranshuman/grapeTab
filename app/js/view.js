...
function moveContentPage() {
    // contentArea를 id로 가는 엘리먼트를 구합니다.
    ...
    navAreaArr.each(function(index){
        $(this).on('click', function(){
            $contentArea.attr('src', './' + mainImageName[index] + '.html');
        });
    });   
}
...
function setNavMovePage(){  
    ...
    $(document).ready(function(){
        ...
        // contentArea를 id로 가는 엘리먼트를 구한 다음
        // popup.js에서 선택한 것에 맞추어 view.html의 iframe에 출력할 페이지 url을 변경합니다.
        $('#contentArea').attr('src', './' + mainImageName[getParameter] + '.html');
    });
}
